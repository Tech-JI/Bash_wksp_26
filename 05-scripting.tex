...
\end{lstlisting}
It's a good practice to enclose variable expansion in double quotes.
\begin{lstlisting}[language=bash]
$ dir='milias lab reports'
$ mkdir $dir    # Creates three folders
$ mkdir "$dir"  # Creates one folder
\end{lstlisting}
\begin{block}
    
Double quotes ensure that bash treats `\$dir' as a single word instead of performing word splitting. 
\footnote{zsh will automatically add double quotes.}
\end{block}
\end{frame}
...
You may encounter "Permission denied" because you didn't have executing permission. 
...
The most important part is the very first column: \verb|-rwxr-xr--|
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Chars 8-10 (Others):} \verb|r--| \\
...
What is the numeric mode for \verb|-rw-r--r--|?
...
$ source test_run.sh # You are now in /tarball
\end{lstlisting}
\end{frame}
...
their usual meaning.
\begin{block}{Note}
    Spaces after \cverb|[[| and before \cverb|]]| are \textbf{required}.
\end{block}
\end{frame}
...
wallet='$$$$$$$$$$$$'
if [[ -z $wallet ]]; then
    echo "I'm broke, spare me some coins please."
fi
\end{lstlisting}
This is equivalent to \cverb|[[ $wallet = '' ]]|. There is also \cverb|-n| for
testing whether a string is not empty, for those who are too lazy to write
\cverb|! [[ -z $wallet ]]|.
\end{frame}
...
\begin{lstlisting}[language=bash]
# for-minimal.sh
for name in John Paul George Ringo; do
...
\cverb|$(COMMAND)| is called \textbf{command substitution}. This is like
variable expansion but for commands. It will be replaced with the output of
\cverb|COMMAND|. Here's an example:
\begin{lstlisting}[language=bash]
# `date` is a command that prints the current time
$ message="Current time: $(date)"
$ echo "$message"
\end{lstlisting}
\begin{block}{Note}
Quotes inside command substitution are isolated from the outside.
\begin{figure}[h]
    \centering
...
Write a script to change all files in the current directory with extension
.jpeg into .jpg extension.
\pause
\begin{block}{Solution}
\begin{lstlisting}[language=bash]
for file in *.jpeg; do
    mv "$file" "$(echo "$file" | sed -E 's/\.jpeg$/.jpg/')"
done
\end{lstlisting}
\end{block}
\end{frame}
...
You have a messy folder filled with thousands of files. Let's clean it up with
the following procedure:
\begin{itemize}
    \item Replace spaces with underscores in filenames
    \item For each file with an extension, create a folder for this extension if such
folder does not exist
    \item Move the file into its corresponding folder
\end{itemize}
Make sure you only organize regular files but not directories, i.e. test
against regular files.
\end{frame}

\begin{frame}[fragile]
\frametitle{The Ultimate Challenge: solution}
\begin{lstlisting}[language=bash]
for file in *; do
    if [[ -f $file ]]; then
        newfile="$(echo "$file" | sed -E 's/ /_/g')"
        mv "$file" "$newfile"
        ext="$(echo "$newfile" | sed -E 's/^.*\.([^.]*)$/\1/')"
        if [[ -n $ext ]] && ! [[ -e $ext ]]; then
            mkdir "$ext"
            mv "$newfile" "$ext"
        fi
...
\newline \newline
It is the perfect place to set environment variables and \textbf{aliases} (custom name).
\begin{lstlisting}[language=bash]
# Edit your ~/.bashrc
$ nano ~/.bashrc
