...
\footnote{
    ls \tt{>} output \tt{2>\&1} works as well. But it's a bit complicated.
}
\end{block}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{xargs (Advanced)}

Pipe operator connects stdout of the previous command and stdin of the next command. But what if the next command doesn't read from stdin?
...
\pause
\begin{block}{Explanation}
    	\tt{xargs} takes input from stdin and converts it into arguments for the next command. In this example, it runs \tt{ls 1 2 3}.
        \footnote{consider why special args in \tt{find . -name "*.txt" -print0 | xargs -0 rm}}
\end{block}
\end{frame}
