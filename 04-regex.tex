...
What about extracting all words starting with A? Deleting all words containing
hyphens?
\newline \newline
If you're sick of this kind of tasks, it's time to appreciate regex and its
companions.
\end{frame}
...
\begin{lstlisting}[language=bash]
$ sed -E 's/([0-9]{1,3}\.){3}[0-9]{1,3}/redacted/g' ipv4
\end{lstlisting}
\begin{block}{Observe}
...
parentheses that mark a region of text that can be used later.
\newline \newline
...
\begin{lstlisting}[language=bash]
$ sed -E 's/(([0-9]{1,3}\.){3})[0-9]{1,3}/\1xxx/g' ipv4
\end{lstlisting}
\begin{block}{Observation}
