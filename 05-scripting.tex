...
Bash, in essence, is a command interpreter, i.e. it executes commands.
We have covered interactive command execution in part one. Here we introduce
script execution of bash.
\newline \newline
...
$ echo $VAR         # access
\end{lstlisting}
\cverb|export| exposes the variable to programs that are not part of the shell
...
\end{lstlisting}
Here \cverb|export| is necessary because \cverb|curl| is not part of the shell
and we need to expose the variable \cverb|HTTPS_PROXY| to it.
\end{frame}
...
shell scripts. The \cverb|chmod| command gives execute permission to
our script, which is necessary.
\end{block}
\end{frame}
...
\begin{block}{Note}
    ...
    \footnote{
        Trivia: In bash the \ctexttt{[[ ]]} is built-in, but in some shells
        it's an executable called \texttt{/usr/bin/[[}.
...
\end{lstlisting}
\begin{block}{Note}
    We do not use \cverb|-eq|, \cverb|-gt| etc.~inside \cverb|(( ))|.
\end{block}
\end{frame}
...
\end{lstlisting}
Complementary operators to \cverb|-z| and \cverb|=|:
\begin{table}
    \centering
    \begin{tabular}{ll}
        \cverb|-n| & not empty \\
...
\end{lstlisting}
Common counterparts to \cverb|-e|:
\begin{table}
    \centering
    \begin{tabular}{ll}
        \cverb|-f| & exists and is regular file \\
        \cverb|-d| & exists and is directory
    \end{tabular}
\end{table}
\end{frame}
...
\end{lstlisting}
\begin{block}{Explanation}
    \cverb|$()| executes a command and takes its output.
...
\newline \newline
Adjust the \textbf{IFS} environment variable to delimit on \cverb|\n| only:
\begin{lstlisting}[language=bash]
...
\end{lstlisting}
\begin{block}{Explanation}
    \cverb|$''| is called ``ANSI-C quoting''. It is not used very often.
    \footnote{
        More on this:
...
\end{lstlisting}
\begin{block}{Explanation}
    \cverb|$(())| is called ``arithmetic expansion''.
\end{block}
\end{frame}
