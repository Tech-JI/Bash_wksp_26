...
Before we introduce regex, \cverb|grep|, and \cverb|sed|, let's do a quick
exercise to get a sense of how it's like without these tools.
\newline \newline
Write a C program to delete all vowels in \texttt{04-regex/literature}.
What about extracting all words starting with A? Deleting all punctuations?
\end{frame}
...
\begin{example}
    \cverb|c.t| matches \cverb|cat|, \cverb|cut|, \cverb|c t|, etc, but not
    \cverb|ct| or \cverb|chat|.
\end{example}

\begin{block}{Note}
    \cverb|\.| matches a literal dot.
\end{block}
\end{frame}
...
\begin{example}
	...
	\item \cverb|^https?://| matches http or https urls \\
...
Extract course codes from \texttt{04-regex/courses}. \newline
...
\begin{frame}[fragile]
\frametitle{Insufficiency of \texttt{grep}}
What if we want to convert the course codes into the modern form?
That is, we append ``0J'' at the end and replace
``VV'' with ``MATH'', ``VY'' with ``ENGL'', ``VG'' with ``ENGR'',
``VC'' with ``CHEM'', etc. \cverb|grep| cannot achieve this easily.
\newline \newline
Enter \cverb|sed|, a program specifically designed for string replacement.
\end{frame}
...
We will be using one very specific syntax for substitution:
\begin{lstlisting}[language=bash]
...
\begin{lstlisting}[language=bash]
$ sed -E 's/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/redacted/g' ipv4
$ sed -E 's/([0-9]+\.){3}[0-9]+/redacted/g' ipv4  # shorter
\end{lstlisting}
\begin{block}{Note}
You can save the output with redirection:
\begin{lstlisting}[language=bash]
$ sed -E 's/([0-9]+\.){3}[0-9]+/redacted/g' ipv4 > ipv4_redacted
\end{lstlisting}
Use the \cverb|-i| flag to modify the input file:
\begin{lstlisting}[language=bash]
$ sed -E 's/([0-9]+\.){3}[0-9]+/redacted/g' ipv4 > ipv4  # BAD
$ sed -i -E 's/([0-9]+\.){3}[0-9]+/redacted/g' ipv4  # GOOD
\end{lstlisting}
\end{block}
\end{frame}
...
Observe what happens when we remove the \cverb|g| at the end in the previous
example.
\begin{lstlisting}[language=bash]
$ sed -E 's/([0-9]+\.){3}[0-9]+/redacted/' ipv4
\end{lstlisting}
...
\textbf{Capturing groups} are a GNU extention to POSIX ERE.
A capturing group, or simply \textbf{group}, is a pattern inside
\cverb|()| that marks a region of text that can be used later.
\newline \newline
It can be accessed with a respective \textbf{backreference} which looks like
\cverb|\1|, \cverb|\2|, up to \cverb|\9|, where \cverb|\1| is the first group,
\cverb|\2| the second, etc.

When nested, the position of \cverb|(| determines order, so the outside group is
\cverb|\1| and the inside one is \cverb|\2|.
\end{frame}
...
\begin{lstlisting}[language=bash]
$ sed -E 's/([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)/\1.\2.\3.xxx/g' ipv4
$ sed -E 's/([0-9]+\.[0-9]+\.[0-9]+\.)[0-9]+/\1xxx/g' ipv4
...
\begin{frame}[fragile]
\frametitle{Your turn}
Now let's solve the problem of converting legacy course codes into modern ones. 
From \tt{04-regex/courses}, extract and convert ``VV'' course codes into
``MATH'' codes.
\begin{example}
    \begin{tabular}{lcl}
        \cverb|VV156	Applied Calculus II| & & \cverb|MATH1560J| \\
        \cverb|VV214	Linear Algebra| & $\Longrightarrow$ & \cverb|MATH2140J| \\
        \cverb|VE215	Introduction to Circuits| & & \textit{unchanged} \\
    \end{tabular}
\end{example}
\pause
\begin{block}{Solution}
\begin{lstlisting}[language=bash]
$ sed -E 's/VV([0-9]{3}).*/MATH\10J/' courses
\end{lstlisting}
\end{block}
\end{frame}
...
Someone constructed \href{https://pdw.ex-parrot.com/Mail-RFC822-Address.html}
{a regex to match email addresses conforming to the RFC 822 standard}. (It's
way more complicated than you would've thought.)

(The above examples are not ERE.)
\end{frame}
...
\href{https://www.grymoire.com/index.html}{The Grymoire} hosts
\href{https://www.grymoire.com/Unix/Regular.html}{a tutorial for regex}
and \href{https://www.grymoire.com/Unix/Sed.html}{for sed}.
\end{frame}
