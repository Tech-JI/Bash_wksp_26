...
\begin{block}{Note}
    You can use backreference not only in replacement strings but also
    in search patterns. E.g.
\begin{lstlisting}[language=bash]
$ echo '"I like backticks."' | sed -E "s/([\"'])(.*)\\1/\`\\2\`/"
\end{lstlisting}
    will replace matching single or double quotes with backticks and output
    \cverb|`I like backticks.`|.
\end{block}
\end{frame}
